...
export default function AdminApproveScreen() {
  ...
  return (
    <ScreenContainer className="flex-1">
      ...
      <Modal visible={showPendingFilter} transparent onRequestClose={() => setShowPendingFilter(false)}><TouchableOpacity activeOpacity={1} onPress={() => setShowPendingFilter(false)} style={{ flex: 1, justifyContent: "center", padding: 28, backgroundColor: "rgba(0,0,0,0.45)" }}><TouchableOpacity activeOpacity={1} onPress={() => {}} style={{ maxHeight: "65%", padding: 20, borderRadius: 16, backgroundColor: "#fff" }}><Text style={{ fontSize: 18, fontWeight: "700", marginBottom: 14 }}>Filter colleague</Text><TouchableOpacity onPress={() => { setPendingStaffId(null); setShowPendingFilter(false); }} style={{ minHeight: 48, justifyContent: "center", paddingHorizontal: 14, borderRadius: 10, backgroundColor: !pendingStaffId ? "#E8F5E9" : "#F5F5F5", marginBottom: 8 }}><Text style={{ fontWeight: "700" }}>All pending</Text></TouchableOpacity><FlatList data={staffProfiles} keyExtractor={(item) => item.uid} renderItem={({ item }) => <TouchableOpacity onPress={() => { setPendingStaffId(item.uid); setShowPendingFilter(false); }} style={{ minHeight: 48, justifyContent: "center", paddingHorizontal: 14, borderRadius: 10, backgroundColor: pendingStaffId === item.uid ? "#E8F5E9" : "#F5F5F5", marginBottom: 8 }}><Text style={{ fontWeight: "700" }}>{item.name}</Text></TouchableOpacity>} /></TouchableOpacity></TouchableOpacity></Modal>
      ...
      <View className="flex-1 mx-3 mt-2 border border-border rounded-xl overflow-hidden"
        ...
        <View style={{ backgroundColor: "#f5f5f5", paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#E5E7EB", flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {!batchMode && pendingRequests.length > 0 && (
            <TouchableOpacity onPress={() => setShowPendingFilter(true)} style={{ position: "absolute", left: "50%", transform: [{ translateX: -44 }], width: 88, paddingHorizontal: 8, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: "#D1D5DB", alignItems: "center" }}><Text numberOfLines={1} style={{ fontSize: 11, fontWeight: "700" }}>{staffProfiles.find((user) => user.uid === pendingStaffId)?.name ?? "Filter"} ⌄</Text></TouchableOpacity>
          )}
          {!batchMode && pendingRequests.length > 0 && (
            <TouchableOpacity
